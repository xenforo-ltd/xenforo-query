 ...
 *
 * Note: Due to the complexity of loading the PHP plugin in test environments, some tests may be integration tests that
 * require manual verification. The PHP PSI structure requires the PHP plugin to be properly initialized.
 */
abstract class XenForoQueryTestCase : BasePlatformTestCase() {
    ...
    /**
     * Configure the test fixture with PHP code. The file must have .php extension for PHP plugin to parse it correctly.
     */
    ...
    protected fun assertCompletionContains(vararg expected: String) {
        ...
        expected.forEach { item ->
            assertTrue("Expected completion '$item' not found. Available: $completions", completions.contains(item))
        }
    }
    ...
    protected fun assertCompletionNotContains(vararg unexpected: String) {
        ...
        unexpected.forEach { item ->
            assertFalse("Unexpected completion '$item' was found in: $completions", completions.contains(item))
        }
    }
    ...
    protected fun assertNoCompletion() {
        val completions = getCompletionStrings()
        assertTrue("Expected no completions but found: $completions", completions.isEmpty())
    }
    ...
    protected fun assertNoReference() {
        val ref = myFixture.getReferenceAtCaretPosition()
        assertTrue("Expected no reference at caret but found: $ref", ref == null || ref.resolve() == null)
    }
    ...
    /**
     * Find a MethodReference in the file at or near the caret position. This searches up the PSI tree from the caret
     * position.
     */
    ...
    /**
     * Check if PHP plugin is properly loaded and PHP files are being parsed. Returns true if the file is recognized as
     * PHP, false if parsed as plain text.
     */
