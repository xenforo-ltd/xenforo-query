...
class XenForoQueryConfigurable(project: Project) : BoundConfigurable("XenForo Query") {
    ...
    override fun createPanel(): DialogPanel = panel {
        group("Features") {
            row { checkBox("Enable table name completion").bindSelected(settings.state::enableTableCompletion) }
            row { checkBox("Enable column name completion").bindSelected(settings.state::enableColumnCompletion) }
            row {
            ...
            row { checkBox("Enable SQL language injection").bindSelected(settings.state::enableSqlInjection) }
            row {
        ...
        group("Table Prefix") {
            ...
            row { checkBox("Only show tables with this prefix").bindSelected(settings.state::requireTablePrefix) }
        }
    }
}
